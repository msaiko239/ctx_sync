...
def process_detach(imsi, ep_address):
    print(f"Attach request for {imsi} in {ep_address}")
    ...
    try:
        ...
        if status_code == 200:
            ...
            for item in data:
                print(f"Subscriber {imsi} status in {ep_address} = ", item['local_ps_attachment'])
                if item['local_ps_attachment'] == "ATTACHED":
                    print(f"Detaching subscriber {imsi} from {ep_address}")
                    ...
                    status_code = c.getinfo(pycurl.RESPONSE_CODE)
                    if status_code == 200:
                        print(f"Subscriber {imsi} detached from {ep_address}")
                    else:
                        print(f"Unable to detach subscriber {imsi} from {ep_address}")
                else:
                    print(f"{imsi} already detached in {ep_address}")
        else:
...
def webhook():
    if request.method == 'POST':
        print(f"Incoming POST from ", request.remote_addr)
        if request.remote_addr == EPC2:
            imsi = request.form.get('imsi')
            if imsi is not None:
                process_detach(imsi, EPC1)
            else:
                print(f"Subscriber {imsi} not found in {EPC1}")
        elif request.remote_addr == EPC1:
            imsi = request.form.get('imsi')
            if imsi is not None:
                process_detach(imsi, EPC2)
            else:
                print(f"Subscriber {imsi} not found in {EPC2}")
        else:
            print("POST request from unknown server or does not match subscriber event", request.remote_addr)
        return '', 200
    else:
        abort(400)
