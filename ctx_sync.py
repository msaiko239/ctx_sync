...
def process_detach(imsi, ep_address, rm_address):
    #app_logger = get_logger()
    app_logger.info(f"Attach request for {imsi} in {rm_address}")
    ...
    try:
        ...
        if status_code == 200:
            ...
            if len(data) != 0:
                ...
                app_logger.info(f"Subscriber {imsi} not found in {ep_address}")
        else:
...
def webhook():
    if request.method == 'POST':
        ...
        if request.remote_addr == EPC2:
            ...
            if imsi is not None:
                process_detach(imsi, EPC1, EPC2)
            else:
                app_logger.info(f"Incorrect event subscriber sent or no imsi found in the data")
        elif request.remote_addr == EPC1:
            imsi = request.form.get('imsi')
            if imsi is not None:
                process_detach(imsi, EPC2, EPC1)
            else:
                app_logger.info(f"Incorrect event subscriber sent or no imsi found in the data")
        else:
